...
export function FileUpload({ onPreviewReady, onPreviewStart, previewFn }: FileUploadProps) {
  ...
  return (
    <div className="w-full">
      <div className="mb-6 rounded-3xl border border-emerald-100 bg-gradient-to-r from-emerald-50 via-white to-teal-50 p-5 lg:p-6">
        <p className="text-sm font-semibold text-emerald-900">Experimental Dataset Intake</p>
        <p className="mt-1 text-sm text-emerald-900/80 leading-relaxed">
          Start with a CSV or Excel file. VivaSense will preview structure, detect likely design factors,
          surface validation warnings, and guide variable mapping before any analysis runs.
        </p>
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-emerald-100 bg-white/80 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Accepted formats</p>
            <p className="mt-1 text-xs text-gray-600">`.csv`, `.xlsx`, `.xls` up to 10 MB.</p>
          </div>
          <div className="rounded-2xl border border-emerald-100 bg-white/80 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Typical columns</p>
            <p className="mt-1 text-xs text-gray-600">Genotype or treatment, replication, environment, and numeric trait columns.</p>
          </div>
          <div className="rounded-2xl border border-emerald-100 bg-white/80 p-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Immediate checks</p>
            <p className="mt-1 text-xs text-gray-600">Rows, variables, likely design, missing values, and column-detection confidence.</p>
          </div>
        </div>
      </div>
      ...
      <label
        ...
        onDrop={onDrop}
        className={[
          "group relative flex flex-col items-center justify-center gap-4 overflow-hidden rounded-3xl border-2 border-dashed p-8 sm:p-10 cursor-pointer transition-all duration-200",
          dragging
          ...
          <>
            ...
            <div className="text-center">
              ...
              <p className="mt-1 text-sm text-gray-500 leading-relaxed max-w-2xl">
                Upload raw field or trial data. VivaSense will infer likely experimental structure and prepare the dataset for scientific review before analysis.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-2 text-xs text-gray-600">
              ...
              <span className="rounded-full border border-gray-200 bg-white px-2.5 py-1">Preview validation</span>
            </div>

            <div className="w-full max-w-2xl rounded-2xl border border-gray-200 bg-white/80 px-4 py-3 text-left">
              <p className="text-xs font-semibold uppercase tracking-wide text-gray-700">Example structure</p>
              <p className="mt-1 text-xs text-gray-600 leading-relaxed">
                `Genotype | Rep | Environment | Yield | DaysToFlowering | PlantHeight`
              </p>
            </div>
