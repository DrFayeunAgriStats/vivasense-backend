...
export function CorrelationHeatmap({ data, mode = "phenotypic" }: CorrelationHeatmapProps) {
  ...
  const hasMatrix = !!stats?.r_matrix && stats.r_matrix.length > 0;
  const pairwiseData = Array.isArray((stats as any)?.pairs) ? (stats as any).pairs : [];
  const hasPairwise = pairwiseData.length > 0;
            ...
            {hasMatrix ? (
              ...
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="min-w-full text-xs text-left">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {pairwiseData.map((pair: any, idx: number) => (
