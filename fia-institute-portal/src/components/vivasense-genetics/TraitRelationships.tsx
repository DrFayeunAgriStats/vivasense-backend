...
import { RegressionAnalysisTab } from "@/components/vivasense/genetics-params/RegressionAnalysisTab";
...
type RelationshipView = "correlation" | "regression";

function toLegacyDatasetContext(ctx: UploadDatasetContext): DatasetContext {
  return {
    file: ctx.file,
...
function extractModeInterpretation(
  fullText: string,
  ...
  const sectionPatterns = {
    phenotypic: ["Phenotypic Correlation", "Phenotypic mode", "field-level"],
    between_genotype: ["Between-Genotype", "between-genotype", "genotype means"],
    genotypic: ["Genotypic Correlation", "Genotypic VC", "variance-component"],
  } as const;

  const allMarkers = [
    ...sectionPatterns.phenotypic,
    ...sectionPatterns.between_genotype,
    ...sectionPatterns.genotypic,
  ];
  const lines = fullText.split("\n");
  const startIndex = lines.findIndex((line) =>
    sectionPatterns[mode].some((marker) => line.toLowerCase().includes(marker.toLowerCase()))
  );
  ...
  if (startIndex >= 0) {
    ...
    for (let index = startIndex + 1; index < lines.length; index += 1) {
      ...
      if (allMarkers.some((marker) => lowerLine.includes(marker.toLowerCase()))) {
        endIndex = index;
        break;
      }
    }
  ...
  const relevant = lines.filter((line) => {
    ...
    return sectionPatterns[mode].some((marker) => lower.includes(marker.toLowerCase())) ||
      line.includes("Pairwise") ||
      line.includes("Multiple Testing") ||
      line.includes("Breeding Decision");
  });
...
function CorrelationMatrixTable({
  ...
  const getColor = (r: number, isDiag: boolean) => {
    if (isDiag) return "transparent";
    const abs = Math.abs(r);
    const alpha = Math.min(abs, 1);
    if (r > 0) return `rgba(29, 158, 117, ${alpha * 0.7})`;
    return `rgba(163, 45, 45, ${alpha * 0.7})`;
  };
  ...
  return (
    <div style={{ overflowX: "auto", marginBottom: 16 }}>
        ...
        <tbody>
          {traitNames.map((rowTrait, i) => (
            <tr key={rowTrait}>
              ...
              >
                {rowTrait.replace(/_/g, " ")}
              </td>
              {traitNames.map((colTrait, j) => {
                const isDiag = i === j;
                const r = isDiag ? 1 : matrix[i]?.[j] ?? null;
                const p = isDiag ? null : pMatrix[i]?.[j] ?? null;
                ...
                return (
                  <td
                    ...
                    style={{
                      padding: "6px 8px",
                      textAlign: "center",
                      background: isDiag ? "#F9FAFB" : getColor(r ?? 0, isDiag),
      ...
      <p style={{ fontSize: 11, color: "#9CA3AF", marginTop: 4 }}>
        * p &lt; 0.05. Green = positive correlation, red = negative.
...
export function TraitRelationships({ datasetContext }: TraitRelationshipsProps) {
  ...
  const [view, setView] = useState<RelationshipView>("correlation");
  ...
  const [interpOpen, setInterpOpen] = React.useState(false);
  ...
  const switcher = (
    <div className="rounded-xl border border-gray-200 bg-white p-2">
      <div className="grid grid-cols-2 gap-2">
        ...
        >
          Regression Analysis{mode !== "pro" ? " (Pro)" : ""}
        </button>
      </div>
    </div>
  );
  ...
  if (view === "regression") {
    return (
      <div className="space-y-5">
        {switcher}
        <RegressionAnalysisTab datasetContext={toLegacyDatasetContext(datasetContext)} />
      </div>
    );
  }
  ...
  if (step === "results" && results) {
    ...
    const modeInterpretation = extractModeInterpretation(results.interpretation, displayMode);
    const activeMatrix = stats?.r_matrix ?? [];
    const activePMatrix = stats?.p_matrix ?? [];
    ...
    return (
      <div className="space-y-5">
          ...
          {hasMatrix && (
            <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
              <div className="mb-3 flex items-center gap-2">
                <h4 className="text-sm font-semibold text-gray-700">Correlation Matrix</h4>
                <span className="text-xs text-gray-400 font-normal">
                  {modeLabel} · bold values are significant at p &lt; 0.05
                </span>
              </div>
              <CorrelationMatrixTable
                matrix={activeMatrix}
                traitNames={results.trait_names}
                pMatrix={activePMatrix}
              />
            </div>
          )}
        ...
        <div style={{ marginTop: 12 }}>
          <button
            type="button"
            onClick={() => setInterpOpen((prev) => !prev)}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 6,
          ...
          >
            <span>{interpOpen ? "▼" : "▶"}</span>
            Academic Interpretation — {modeLabel}
          </button>
          {interpOpen && (
            <div
              style={{
            ...
            >
              {modeInterpretation || "No interpretation available for this mode."}
            </div>
          )}
        </div>
      ...
      {showPreview && (
        <WordExportPreviewModal
          ...
          sections={buildCorrelationPreview(results, displayMode, stats!, (results.method === "spearman" ? "spearman" : "pearson"), userObjective).sections}
          warnings={buildCorrelationPreview(results, displayMode, stats!, (results.method === "spearman" ? "spearman" : "pearson"), userObjective).warnings}
          notes={buildCorrelationPreview(results, displayMode, stats!, (results.method === "spearman" ? "spearman" : "pearson"), userObjective).notes}
          canExport={canExport}
