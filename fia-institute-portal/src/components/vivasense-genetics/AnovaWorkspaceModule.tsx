...
import { AnovaStructurePreview } from "@/components/vivasense-genetics/AnovaStructurePreview";
import { SplitPlotExplanationPanel } from "@/components/vivasense-genetics/SplitPlotExplanationPanel";
...
export function AnovaWorkspaceModule({ datasetContext }: AnovaWorkspaceModuleProps) {
  ...
  // categoricalColumns must be declared before the useEffect that references it.
  const categoricalColumns = useMemo(() => {
    if (!datasetContext) return [];
    const traits = new Set(datasetContext.availableTraitColumns);

    // Use ALL dataset columns, not only genotype/rep/environment
    const allColumns =
      (datasetContext as any).columns ??
      (datasetContext as any).availableColumns ??
      [];

    return allColumns.filter(
      (col: string) => !traits.has(col)
    );
  }, [datasetContext]);

  const factorAOptions = useMemo(
    () => categoricalColumns.filter((col) => col !== repColumn),
    [categoricalColumns, repColumn]
  );

  const factorBOptions = useMemo(
    () => categoricalColumns.filter((col) => col !== repColumn && col !== factorA),
    [categoricalColumns, repColumn, factorA]
  );

  const factorCOptions = useMemo(
    () => categoricalColumns.filter((col) => col !== repColumn && col !== factorA && col !== factorB),
    [categoricalColumns, repColumn, factorA, factorB]
  );

  // REQUIRED IMPLEMENTATION: Create centralized condition
  const hideTreatmentSelector = design === 'split_plot_rcbd' || design === 'factorial';
  ...
  useEffect(() => {
    ...
    const columnAnalysis: ColumnAnalysis[] = categoricalColumns.map(col => ({
      name: col,
      uniqueValues: 0, // Would need actual data to compute this
      isNumeric: false,
      sampleValues: [],
    }));
  ...
  // TASK 1: Simplified cleanup logic — clear treatment state when selector is hidden
  useEffect(() => {
  ...
  // Clear design-specific fields when switching away from a design
  ...
  useEffect(() => {
    ...
    if (prev === "split_plot_rcbd") { setMainPlot(""); setSubPlot(""); }
    if (prev === "factorial") { setFactorA(""); setFactorB(""); setFactorC(""); }
    setResults(null);
    setError(null);
  }, [design]);
  ...
  const valid =
    ...
    (hideTreatmentSelector || treatmentColumn) && // treatmentColumn is only required if not hidden
    (design === "crd" || repColumn) &&
    (design !== "factorial" || (factorA && factorB && factorA !== factorB && (!factorC || (factorA !== factorC && factorB !== factorC)))) &&
    (design !== "split_plot_rcbd" || (mainPlot && subPlot && repColumn && mainPlot !== subPlot));
  ...
  const run = async () => {
    ...
    setError(null);
    setResults(null);
    // Debug: log design before upload
    console.log("Selected Design:", design);
    try {
      const data = await analyzeUpload({
        base64_content: datasetContext.base64Content,
        file_type: datasetContext.fileType,
        genotype_column: hideTreatmentSelector ? undefined : treatmentColumn || undefined,
        rep_column: design === "crd" ? undefined : repColumn || undefined,
        ...
        module: "anova",
        design_type: design,
        treatment_column: hideTreatmentSelector ? undefined : treatmentColumn || undefined,
        factor_a_column: design === "factorial" ? factorA : undefined,
        factor_b_column: design === "factorial" ? factorB : undefined,
        factor_c_column: design === "factorial" && factorC && factorC !== "None" ? factorC : undefined,
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4">
        ...
        <div className="mb-4">
          <Field label="Design" required>
            <select
              value={design}
              onChange={(e) => {
                const newDesign = e.target.value as AnovaDesign;
                setDesign(newDesign);

                // Selective column reset logic
                if (newDesign === 'split_plot_rcbd') {
                  setTreatmentColumn("");
                }
                if (newDesign === 'crd') {
                  setRepColumn("");
                ...
                // Never clear: uploaded dataset, response variables, replication count, alpha level
              }}
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          {/* Split-Plot Design: Show Main Plot and Sub-Plot factors in summary panel */}
          {design === "split_plot_rcbd" && (
            <>
              <Field 
            ...
            <>
              ...
              >
                <ColumnSelect 
                  value={factorB} 
                  onChange={setFactorB} 
                  options={factorBOptions}
                  placeholder={getColumnPlaceholder("factorB", design, domain)}
                />
              </Field>
              <Field
                label={(labels as any).factorCLabel || "Factor C (optional)"}
                helpText={getFieldHelpText("factorC" as any, design)}
              >
                <select
                  value={factorC}
                  onChange={e => setFactorC(e.target.value)}
                  className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm"
                >
                  <option value="">None</option>
                  {factorCOptions.map((option) => (<option key={option} value={option}>{option}</option>))}
                </select>
              </Field>
            </>
          )}
        ...
        <div className="mt-4">
          <AnovaStructurePreview design={design} />
        </div>
      ...
      {results && (
        <ErrorBoundary label="Results Display">
          <ResultsDisplay
            results={results}
