...
export function GeneticsWorkspaceModule({ datasetContext }: GeneticsWorkspaceModuleProps) {
  ...
  const [selectionIntensity, setSelectionIntensity] = useState<number>(DEFAULT_SELECTION_INTENSITY);
  const [running, setRunning] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [results, setResults] = useState<UploadAnalysisResponse | null>(null);

  // Domain-aware logic
  const domain = datasetContext?.research_domain ?? "plant_breeding";
  const isPlantBreeding = domain === "plant_breeding";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4">
        ...
        {/* Selection Intensity: Only show for plant breeding domain */}
        {isPlantBreeding && (
          <div className="mb-4">
            <div className="mb-1 flex items-center gap-1.5">
              ...
              <button
                ...
                title="Selection intensity (i) is the standardised selection differential. It determines how aggressively superior genotypes are selected. Default is 20% (i = 1.400) per Falconer & Mackay (1996)."
